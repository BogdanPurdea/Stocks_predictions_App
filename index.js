...
async function fetchStockData() {
    ...
    try {
        const stockData = await Promise.all(tickersArr.map(async (ticker) => {
            const baseUrl = 'https://polygon-api-worker.bogdanpurdea14.workers.dev/'
            const url = `${baseUrl}?ticker=${encodeURIComponent(ticker)}&startDate=${encodeURIComponent(dates.startDate)}&endDate=${encodeURIComponent(dates.endDate)}`
...
async function fetchReport(data) {
    ...
    try {
        const url = 'https://openai-api-worker.bogdanpurdea14.workers.dev/'
